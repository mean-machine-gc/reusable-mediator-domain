...
**Commands:**
- `createMediation` — create a new Mediation in Draft state with source topic, destination, and pipeline steps
- `updatePipeline` — replace the pipeline steps/rules (only in Draft or Deactivated)
- `activateMediation` — Draft → Active (must verify no other Active Mediation exists for same topic+destination)
- `deactivateMediation` — Active → Deactivated
